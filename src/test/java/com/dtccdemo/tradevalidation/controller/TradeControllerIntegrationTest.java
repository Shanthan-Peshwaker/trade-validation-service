...
class TradeControllerIntegrationTest {
    ...
    void standardizesAliasFieldNamesFromADifferentSourceSystem() throws Exception {
        // dd/MM/yyyy format, computed relative to today so this test doesn't
        // start failing once a hardcoded date falls outside the validator's
        // allowed settlement-date window.
        String valueDate = LocalDate.now().plusDays(5).format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        String body = """
                {
